...
<style>
  ...
  .badge-topic {{
    background: rgba(53,206,141,0.15);
    color: var(--text);
    border-color: rgba(53,206,141,0.35);
  }}
  .badge-outlet {{
  ...
  .badge-score-low {{
    ...
    color: var(--text);
    border-color: rgba(107,162,146,0.5);
  }}
  .badge-score-mid {{
    background: rgba(53,206,141,0.28);
    color: var(--text);
    border-color: rgba(53,206,141,0.5);
  }}
  .badge-score-high {{
    background: rgba(5,82,88,0.28);
    color: #fff;
    border-color: rgba(5,82,88,0.6);
  }}
  .badge-score-top {{
  ...
  [data-testid="stSlider"] > div > div > div > div {{
    background: rgba(53,206,141,0.35);
  }}
  [data-testid="stSlider"] .st-bk {{
    background: rgba(53,206,141,0.35);
  }}
  [data-testid="stSlider"] .st-c0 {{
...
with st.sidebar:
    ...
    include_outlets = st.text_input("Include outlets (comma-separated)", placeholder="e.g., Reuters, The Verge")
    exclude_outlets = st.text_input("Exclude outlets (comma-separated)", placeholder="e.g., CNN, BBC News")
    min_articles = st.slider("Min articles per reporter", 1, 10, 1)
...
if run:
    ...
    includes = set(normalize_list(include_outlets))
    excludes = set(normalize_list(exclude_outlets))
    if includes:
        articles = [a for a in articles if (a.get("source","").strip().lower() in includes)]
    if excludes:
        articles = [a for a in articles if (a.get("source","").strip().lower() not in excludes)]
    ...
    with tab_reporters:
        ...
        for author, info in reporters.items():
            ...
            if profile and profile.get("topics"):
                topics = list(dict.fromkeys([t for t in profile["topics"] if isinstance(t, str)]))[:8]
            else:
                words = []
                for art in top_articles:
                    title = (art.get("title") or "")
                    words += re.findall(r"[A-Za-z][A-Za-z\-]{2,}", title)
                common = [w for w, c in Counter([w.lower() for w in words]).most_common(30) if w.lower() not in {"the","and","for","with","from","into","over","under","about","into","your","our","are","was","were","has","have","will","can","new"}]
                topics = common[:8]

            rows.append({
                "Reporter": author, "Outlets": outlets_str, "ArticlesMatched": len(arts),
                "Score": score, "TopArticles": top_articles, "Email": email,
                "Profile": profile, "Topics": topics
            })
        ...
        if not rows:
            ...
            for _, r in df.reset_index(drop=True).iterrows():
                ...
                if r.get("Topics"):
                    st.markdown("".join([f'<span class="badge badge-topic">{t}</span>' for t in r["Topics"]]), unsafe_allow_html=True)
            ...
            with pd.ExcelWriter(output, engine="openpyxl") as writer:
                ...
                } for r in rows]).sort_values(by=["Score","ArticlesMatched"], ascending=False)
                rep_df.to_excel(writer, index=False, sheet_name="Reporters")
                art_rows = []
                for r in rows:
                    for a in r["TopArticles"]:
                        art_rows.append({
                            "Reporter": r["Reporter"],
                            "ArticleTitle": a.get("title"),
                            "URL": a.get("url"),
                            "Outlet": a.get("outlet"),
                            "Published": a.get("published"),
                        })
                pd.DataFrame(art_rows).to_excel(writer, index=False, sheet_name="TopArticles")
            st.download_button("📥 Download results (XLSX)", data=output.getvalue(), file_name="reporter_results.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
